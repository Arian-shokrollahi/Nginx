...
2. ا<mark>-**Block context**:</mark> بلوک‌های ساختاری که با آکولاد (`{ }`) باز و بسته می‌شوند و یک محیط یا **Context** برای سایر دستورات می‌سازند (مثل `http { }`، `server { }` و `location { }`).
